...
public:
	...
	// 将来的にGetInputNumsと統合したい
	template <typename T>
	std::vector<T> GetInputNumLines(int num);
...
template <typename T>
std::vector<T> InputManager::GetInputNumLines(int num) {
	std::vector<T> input;
	if (m_isCheckMode) {
		repeat(i, num) {
			std::istringstream s(PopInputLine());
			T tmp;
			s >> tmp;
			input.push_back(tmp);
		}
...
int main() {
	Questionb234B::AtCoderQuestion question;
	if (IS_CHECK) {
		//if (question.Check()) {
